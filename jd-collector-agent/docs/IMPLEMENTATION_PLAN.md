...
- 추가 보정 진행:
  - JobKorea 캡처 우선순위 개선
  - OCR 전처리 개선
  - `jd-research-tool` 복합공고 `roles[]` 구조 도입
  - `capture_failed` 판정 완화 (one_image_short_text 소프트 경고)
  - `utils.py` 개발/이공계 편향 제거 (Top8 대분류 기준 정렬)
  - **복합공고 `roles[]` DB 저장 구조 + 백필 완료**
  - **collector 전용 브라우저 프로필 기반 로그인 세션 유지 추가**
- 현재 핵심 미완료:
  - `classify.py`의 top8 기준 전환 (Phase 4)
  - collector payload 흐름 통합 리팩토링
    - 현재는 `job_postings` / `job_sections` / 품질 판정에는 normalized payload 사용
    - `job_posting_roles` 저장에는 원본 structured payload 사용
    - 이유: `ensure_enriched_schema()`가 `roles[]`, `common_requirements`, `common_preferred`를 완전히 보존하지 못하기 때문
    - 즉 지금은 문제를 막기 위한 임시 우회 구조이고, 나중에 한 번 정리해야 한다
